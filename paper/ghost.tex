...
without looking at micro-management moves like kiting. This problem has been studied by Furtak and Buro~\cite{Furtak2010} as an {\em attrition game} where they proved that this problem is PSPACE.
...
linear splash, where the are of damage is a line to the target; and the radial splash, where the are of damage is a circled
...
\end{itemize}
%--------------
Notice that these two objective are  more complex than looking for the
...
\begin{table*}[ht]
  ...
  \begin{tabular}{|c|l|c|c|c|c|c|c|} 
    ...
    \cline{2-8}
    \multicolumn{1}{c|}{}& {\em Objective} &  Win \% & \# Draws & \#  Avg. living units &
...
time. Eventually,  the two groups can  kill each other, leading  to a
...
of 5~ms.   This is  why  we have  chosen to  set the  optimization
timeout to  3~ms and 5~ms to be able to compare our results with  the same timeout
...
efficient with a  win rate of 82\% within 5~ms,  (80\% within 3~ms). One
...
% in~\cite{Perkins10}.
The exceptions are the  works  dealing  with the  wall-in  problem like the
the one by Certicky~\cite{Certicky13} and the  wall-in solver
by Richoux  et al.~\cite{RichouxUO14}. As we already said, the
latter provides the basis for the present work.
...
In this paper, we  will call {\it
  significant gap} a gap that allows a Zerglings (a unit of size $16 \times 16$ pixels) to
cross a wall.
...
disfavored since their global timeout is 10~ms shorter than satisfaction
runs (150~ms). For optimization  runs, $x$ and $y$ parameters where
...
unable  to  produce a  unit because  we  reached the  supply
capacity limit).
...
performances.   Secondly,   Opturion   CPX   and  Gecode   are   well   known
...
Secondly, Oscar/CBLS is  a very recent
...
(DPS) on  ground units,  without taking  into account  splash damage.
...
minute and then collected for each run the runtime when it hit its highest
...
find  the optimal  solution  in  200~ms, \ie,  2.5  times longer than  \ghost
...
Terran  instance. While  \ghost finds  53\% of  the time  the optimal
...
because Zerg  units' properties make  the Zergling unit  far more
...
allow you to do. This  is not the case with the  Protoss instance where a
...
instance rather  than the  Protoss instance,  is  because Terrans
...
We should also emphasize that Oscar/CBLS uses many cores to compute a solution\footnote{Experiments have been conducted on an Intel i7 quad-core CPU, with 4GB of memory, under Ubuntu 14.04 64-bit.}, whereas \ghost inner solver remains sequential. Besides this difference, results compiled in Table~\ref{tab:oscar} show that \ghost outperforms Oscar/CBLS both in term a quality outputs and runtimes.
}
...
Some  improvements we  have in  mind concern to the implementation  of a
