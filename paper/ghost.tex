...
\begin{abstract}
  This paper  presents \ghost, a combinatorial  optimization solver where an
...
current situation  and then to take the right decision. This is  especially true
...
design  Artificial  Intelligence (AI) techniques  that  may  be  applied
...
Bayesian programming, goal-driven autonomy or  potential fields. We
...
\item {\textbf Tactics}:  are the implementation of  the current strategy.
  It includes, among other things, army and building  positioning, movements, timing. Tactics concerns a group of units.
...
StarCraft:  Brood War  is the  expansion pack for the game  StarCraft, both
...
A player has to  gather two  kind of resources,  minerals and  gas, in
order   to  construct   buildings. Those buildings are  necessary to produce units,   upgrade
characteristics such as damage points or armor and unlock technologies
and special abilities. The production of units has also a cost, each unit type
has their own characteristics (hit points, damage points, size or abilities)
and an adequate cost.

The game  is played on  a rectangular map,  discretized into two  kind of
tiles: \emph{walk  tiles}, composed of  $8 \times  8$ pixels and  \emph{build tiles}
composed of $4  \times 4$ \emph{walk tiles}, \ie, $32  \times 32$ pixels. The
...
every RTS game, the map is under a fog of war, only dissipated
...
StarCraft has  been a world wide  success, and it is still  the most sold RTS game
of the history with nearly 10 millions of copies distributed\footnote{\url{http://www.msnbc.msn.com/id/18925251/}}
. It has been  and  still is  particularly  popular  in  South Korea,  where  a
...
time's flow  (15 logic frames per second)  and the 6 others distribute
it. This is why, when we will  write about in-game time all along this
paper, we will refer to the terms \emph{StarCraft unit time} rather than
seconds, in order to avoid any confusions with proper seconds. In the fastest
mode, one logical game frame takes 42ms. In StarCraft AI  competitions, it is
...
the vector $V^k$, we say that $c$ is either  \emph{satisfied} (equals to
$\top$) or  \emph{unsatisfied} (equals to $\bot$).
...
$C$ represent the same sets as a \csp, and $f$ is an  \emph{objective
...
is called a  \emph{configuration}.  Since
...
Diaz~\cite{Codognet01}, to be the heart
of   the   solver. The reason we have chosen a meta-heuristics is
...
tree-based search algorithms. The reason  we have chosen  \emph{Adaptive
...
the tabu list.  Indeed,  \emph{Adaptive  Search} contains a tabu list of
...
shoot. Liu et al. in~\cite{liuusing} using potential fields, influence maps 
and genetic algorithms, to tune the parameters, to solve kiting, target
selection and flee decisions.
And Churchill  et  al.  in~\cite{ChurchillSB12,ChurchillB12}
...
satisfaction version  is very easy.  It consists on assign  to each
...
and combined  it with \ghost. It  was then faster to  develop a quick
...
time. It is possible for the two groups to  kill one another, leading  to a
...
like Zerglings, Marines  or Zealots. In this paper, we  will call 
  \emph{significant gap} a gap allowing Zerglings ($16 \times 16$ pixels) to
enter into the base.
...
outside-the-book  strategies,  and  thus  minimizing  the  build  order
...
all of them to speed up the  search. This can also be done easily since
  \emph{Adaptive  Search}   is  known  to  be  very   efficient  with  a
